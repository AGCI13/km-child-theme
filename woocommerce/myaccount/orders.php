...
<?php if ($has_orders) : ?>
	...
	<div class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
		<div class="entete-tab-commande">
			<div class="titre-entete-commande">Commande</div>
			<div class="titre-entete-date">Date</div>
			<div class="titre-entete-etat">État</div>
			<div class="titre-entete-total invisible-on-mobile">Total</div>
			<div class="titre-entete-facture invisible-on-mobile">Facture</div>
			<!-- <?php //foreach (wc_get_account_orders_columns() as $column_id => $column_name) : 
					?>
				<th class="woocommerce-orders-table__header woocommerce-orders-table__header-<?php // echo esc_attr($column_id); 
																								?>"><span class="nobr"><?php //echo esc_html($column_name); 
																														?></span></th>
			<?php //endforeach; 
			?> -->
		</div>
		...
		<div class="bloc-commandes">
			...
			?>
				<div style="display:flex; margin-bottom:5%; margin-top:2%; align-items: center;" class="woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr($order->get_status()); ?> order">
					<?php foreach (wc_get_account_orders_columns() as $column_id => $column_name) : ?>
						<div class="colonne-commande woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr($column_id); ?>" data-title="<?php echo esc_attr($column_name); ?>">
